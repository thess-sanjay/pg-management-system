...
public class RentRecordService {
 ...
 public RentRecord updateRentRecord(
     ...
     if (updatedRent.getRentAmount() <= 0) {
     ...
     // Update rent amount
     existingRent.setRentAmount(
             updatedRent.getRentAmount()
     );

     // Update due date if provided
     if (updatedRent.getDueDate() != null) {
     ...
     // If existing record has no due date,
     // calculate it from tenant joining date
 ...
 // Get rent records due today
 public List<RentRecord> getTodayDueRentRecords() {

     LocalDate today = LocalDate.now();

     return rentRecordRepository.findByDueDateAndStatus(
             today,
             "PENDING"
     );
 }
